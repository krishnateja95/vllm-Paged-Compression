...
CACHE_TYPES=("percentage")

BLOCK_SIZES=(16)
TP=1

BENCHMARK_SCRIPT_DIR="$HOME/kv_evict/vllm-Paged-Compression/benchmarks/"
LOG_DIR="$HOME/kv_evict/perf_logs/various_seqlens_tp${TP}/"
...
run_percentage_benchmark_with_evictsize() {
    ...
    [ "$MODEL_NAME" = "Qwen2.5-7B-Instruct-1M" ] && max_model_len="--max-model-len 131072"
...
echo "Start the various INPUT/OUTPUT length benchmark..."
echo "===================================================================="
...
for CACHE_TYPE in "${CACHE_TYPES[@]}"; do
    if [ "$CACHE_TYPE" = "percentage" ]; then
        ...
        for INPUT_OUPUT_LEN in "${INPUT_OUPUT_LENS[@]}"; do
            ...
            echo "Running benchmarks with input/output length: $INPUT_LEN"
            for P_AND_D_EVICT_METHOD in "${PROMPT_AND_DECODE_EVICT_METHODS[@]}"; do
                ...
                for NUM_REQ in "${NUM_REQS[@]}"; do
                    ...
                    for MODEL in "${MODELS[@]}"; do
                        ...
                        if [ $USE_EVICT_FREQ -eq 1 ]; then
                            for BLOCK_SIZE in "${BLOCK_SIZES[@]}"; do
                                for EVICT_FREQ in "${EVICT_FREQS[@]}"; do
                                    LOG_FILE_NAME="result_m${MODEL_NAME}_b${BLOCK_SIZE}_ef${EVICT_FREQ}_r${NUM_REQ}_p${INPUT_LEN}_g${OUTPUT_LEN}_tp${TP}.log"
                        ...
                        echo -e "\Finshed the test with $MODEL model..."
                    done
        ...
        echo "Start the full-cache (i.e., KV cache eviction disabled) benchmark test ..."
        echo "Running benchmarks with KV cache eviction disabled (i.e. using full cache)"
        # Run the benchmark with KV eviction disabled
        TMP_LOG_DIR="${LOG_DIR}/full_cache/"
        # check if the TMP_LOG_DIR1 exists; if not, create it
        create_log_dir "$TMP_LOG_DIR"
        ...
        for INPUT_OUPUT_LEN in "${INPUT_OUPUT_LENS[@]}"; do
            ...
            for NUM_REQ in "${NUM_REQS[@]}"; do
                ...
                for MODEL in "${MODELS[@]}"; do
                    ...
                    echo -e "\Start the test with $MODEL model..."  
                    for BLOCK_SIZE in "${BLOCK_SIZES[@]}"; do
                        LOG_FILE_NAME="result_m${MODEL_NAME}_b${BLOCK_SIZE}_r${NUM_REQ}_p${INPUT_LEN}_g${OUTPUT_LEN}_tp${TP}.log"
                        run_fullcache_benchmark "$MODEL" "$NUM_REQ" "$BLOCK_SIZE" "$TMP_LOG_DIR" "$LOG_FILE_NAME"
                        sleep 1
                    done
