...
# EVICT_METHODS=("default" "streamingLLM-1" "local" "global" "inverse_key_l2")
EVICT_METHODS=("streamingLLM-1" "global" "local" "inverse_key_l2")
...
BASE_LOG_PATH="$HOME/kv_evict/perf_logs/latency_test/"
...
run_kv_evict_benchmarks_ttft() {
    local model=$1 evict_method=$2 num_req=$3 input_len=$4 output_len=$5 evict_log_path=$6 log_file_name=$7
        ...
        for model in "${MODELS[@]}"; do
            ...
            for num_req in "${NUM_REQS[@]}"; do
                for INPUT_OUTPUT_LEN in "${INPUT_OUTPUT_PAIRS[@]}"; do
                    ...
                    log_file_name="${model_name}_r${num_req}_p${input_len}_g${output_len}_ttft.log"
                    run_fullcache_benchmark_ttft "$model" "$num_req" "$input_len" "$output_len" "$full_cache_log_path" "$log_file_name"
                done
            done
            echo "==================================================================="
        done
        ...
        for model in "${MODELS[@]}"; do
            ...
            for num_req in "${NUM_REQS[@]}"; do
                for INPUT_OUTPUT_LEN in "${INPUT_OUTPUT_PAIRS[@]}"; do
                    ...
                    if [ "$evict_method" = "local" ]; then
                        log_file_name="${model_name}_r${num_req}_p${input_len}_g${output_len}_k${TOPK_BLOCKS}_ttft.log"
                    else
                        log_file_name="${model_name}_r${num_req}_p${input_len}_g${output_len}_ttft.log"
                    fi
                    run_kv_evict_benchmarks_ttft "$model" "$evict_method" "$num_req" "$input_len" "$output_len" "$evict_log_path" "$log_file_name"
                    echo "********************************************************************"
                done
            done
            echo "==================================================================="
