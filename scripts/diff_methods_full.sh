...
EVICT_METHODS=("streamingLLM-1" "local" "global")

CACHE_BUDGETS=(1024 2048 4096)
# CACHE_BUDGETS=(256 512)
...
BASE_LOG_PATH="$HOME/kv_evict/perf_logs/"
...
run_kv_evict_benchmarks_with_budgets() {
    local model=$1 evict_method=$2 cache_budget=$3 num_req=$4 input_len=$5 output_len=$6 evict_log_path=$7 log_file_name=$8
    ...
    echo -e "\tnum_req: $num_req, input_len: $input_len, output_len: $output_len"
...
run_fullcache_benchmark() {
    local model=$1 num_req=$2 input_len=$3 output_len=$4 full_cache_log_path=$5 log_file_name=$6
    local model_name=`echo ${model} | cut -d '/' -f 2`

    echo -e "\tProcessing $num_req requests (inputlen=$input_len, outputlen=$output_len) with $model model..."
        ...
        for model in "${MODELS[@]}"; do
            ...
            for num_req in "${NUM_REQS[@]}"; do
                for INPUT_OUTPUT_LEN in "${INPUT_OUTPUT_PAIRS[@]}"; do
                    ...
                    log_file_name="${model_name}_r${num_req}_p${input_len}_g${output_len}.log"
                    run_fullcache_benchmark "$model" "$num_req" "$input_len" "$output_len" "$full_cache_log_path" "$log_file_name"
                done
            done
            echo "==================================================================="
        done
            ...
            for num_req in "${NUM_REQS[@]}"; do
                for INPUT_OUTPUT_LEN in "${INPUT_OUTPUT_PAIRS[@]}"; do
                    ...
                    for cache_budget in "${CACHE_BUDGETS[@]}"; do
                        log_file_name="${model_name}_bd${cache_budget}_r${num_req}_p${input_len}_g${output_len}.log"
                        if [ "$evict_method" = "local" ]; then
                            log_file_name="${model_name}_bd${cache_budget}_r${num_req}_p${input_len}_g${output_len}_k${TOPK_BLOCKS}.log"
                        else
                            log_file_name="${model_name}_bd${cache_budget}_r${num_req}_p${input_len}_g${output_len}.log"
                        fi
                        run_kv_evict_benchmarks_with_budgets "$model" "$evict_method" "$cache_budget" "$num_req" "$input_len" "$output_len" "$evict_log_path" "$log_file_name"
                        echo "********************************************************************"
                    done
                done
            done
