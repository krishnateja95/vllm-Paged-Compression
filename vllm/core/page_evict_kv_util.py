...
def get_num_required_blocks_after_prune_promt(q_len: int, paged_evict_config: PagedEvictConfig):
    ...
    if paged_evict_config.prompt_evict_method == "streamingLLM":
        if paged_evict_config.cache_prune_type == "percentage":
            remainder_size = q_len % paged_evict_config.compressed_block_size
            
            end_first_slice_idx = paged_evict_config.initial_blocks * paged_evict_config.compressed_block_size
            num_first_slice_tokens = end_first_slice_idx
            
            end_middle_slice_idx = q_len - paged_evict_config.compression_rate * paged_evict_config.compressed_block_size - remainder_size
            ...
            pruned_middle_tokens = num_middle_slice_tokens - int(num_middle_slice_tokens/paged_evict_config.compression_rate) 
            # print(f"****1======prunning prompt with percentage using streamingLLM: pruned_middle_tokens={pruned_middle_tokens}")
            pruned_middle_tokens = paged_evict_config.compressed_block_size * (math.floor(pruned_middle_tokens/paged_evict_config.compressed_block_size))
            # print(f"****1======prunning prompt with percentage using streamingLLM: pruned_middle_tokens={pruned_middle_tokens}") 
            ...
            return cdiv(total_unpruned_tokens, paged_evict_config.compressed_block_size), total_unpruned_tokens
        else:
            ...
            end_middle_slice_idx = q_len - paged_evict_config.compression_rate * paged_evict_config.original_block_size - remainder_size
            num_middle_slice_tokens = end_middle_slice_idx - end_first_slice_idx
            ...
            middle_unpruned_tokens = paged_evict_config.cache_budget - \
                ...
                (paged_evict_config.compression_rate * paged_evict_config.original_block_size)
...
def get_blocks_to_prune(paged_evict_config: PagedEvictConfig, seq_kv_len: int):    
    if paged_evict_config.decode_evict_method == "streamingLLM":
        if paged_evict_config.cache_prune_type == "percentage":
            ...
            e_block_id = s_block_id + int(paged_evict_config.evict_size / paged_evict_config.compressed_block_size)
            pruned_tokens = paged_evict_config.evict_size
        else:
        ...
        # our algorithm
        if paged_evict_config.cache_prune_type == "percentage":
            s_block_id = paged_evict_config.initial_blocks + 1 # add 1 since we need to write the prunned key-value back to the first block
            e_block_id = paged_evict_config.initial_blocks + int(paged_evict_config.original_block_size / paged_evict_config.compressed_block_size)
            pruned_tokens = paged_evict_config.evict_size
        else:
            # For cache_budget case
            if seq_kv_len <= paged_evict_config.cache_budget:
